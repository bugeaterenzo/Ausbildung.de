...
public class Create_Url_File {
    ...
    static final String OUTPUT_FILE_PATH = "AusbildungData.txt"; // Example file name
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter(OUTPUT_FILE_PATH, true)); // 'true' appends to file
        ...
        for (int i = 0; i < List_Of_Ausbildung_Articles.size(); i++) {
            ...
            // Write to file in the key-value format (CompanyName: URL)
            writer.write(company_name + ": " + New_URL);
            writer.newLine(); // Adds a new line after each entry

            System.out.println(i + ". company_name_and_address " + company_name + " with the URL _ : ");
            System.out.println(New_URL + " was added to the file.");

            switch_back(driver);
        }

        // Close the writer after all the entries are added
        writer.close();

        System.out.println("Data has been written to " + OUTPUT_FILE_PATH);
    }

    // Your existing switch_tabs() and switch_back() methods...
